...
function isSystemUpdateLog(log: WorkLogCardProps["log"]) {
  return log.type === "update" && (log.title.startsWith("事项变化：") || log.title.startsWith("浜嬮」鍙樺寲"));
}

function normalizeSystemTitle(title: string) {
  if (title.startsWith("事项变化：")) return title.replace("事项变化：", "");
  return title;
}
    ...
    <div className={`card card-hover entity-card work-log-card ${typeClass}`}>
      <div className="log-card-topline entity-card-header">
        <span className="log-type-icon">
          <Icon name={isSystemUpdate ? "refresh" : typeIcons[log.type] || "file-text"} size={15} />
        </span>
        <div className="log-card-heading entity-card-body">
          <div className="log-card-badges entity-card-badges">
            <span className={`badge badge-${log.type}`}>
              {isSystemUpdate ? "系统动态" : WORK_LOG_TYPE_LABELS[log.type] || log.type}
            </span>
            <span className="log-date">{log.workDate}</span>
            {!isSystemUpdate && reportable && <span className="badge badge-reportable">可汇报</span>}
            {!isSystemUpdate && evidenceLabel && <span className="entity-pill entity-pill--blue">{evidenceLabel}</span>}
            {!isSystemUpdate && isHighValue && !evidenceLabel && <span className="entity-pill entity-pill--blue">关键事实</span>}
            {!isSystemUpdate && <span className="log-source">{SOURCE_LABELS[log.source] || log.source}</span>}
          </div>
          <h3 className="entity-card-title">
            {isSystemUpdate ? normalizeSystemTitle(log.title) : log.title}
          </h3>
      ...
      <div className="entity-card-meta log-card-meta">
        ...
        {!isSystemUpdate && log.sourceUrl && <span>来源证据</span>}
      </div>
    </div>
  );

  return showLink ? (
    <Link href={`/logs/${log.id}`} className="card-link">
      {content}
    </Link>
  ) : (
    content
